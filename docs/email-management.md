...
The old single-account Email Settings screen is gone. Its account was carried over automatically
by the `drop_email_settings_table` migration — nothing sends until at least one account exists
here, so an installation that was sending before the module landed keeps sending after it.
