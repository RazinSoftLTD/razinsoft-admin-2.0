...
class CheckoutController extends Controller
{
    ...
    /**
     * Confirm a returned Stripe session and fulfil the order (fallback to the webhook).
     * Called by the success page after Embedded Checkout completes.
     */
    public function confirm(Request $request, OrderService $orders, PaymentService $payments, string $orderNumber)
    {
        $order = $request->user()->orders()->where('order_number', $orderNumber)->firstOrFail();

        if (! $order->isPaid() && $payments->stripeSessionPaid($order, $request->query('session_id'))) {
