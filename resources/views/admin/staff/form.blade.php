...
@section('content')
    ...
    <form method="POST" action="{{ $staff->exists ? route('admin.staff.update', $staff) : route('admin.staff.store') }}" enctype="multipart/form-data" class="max-w-2xl">
        ...
        <div class="space-y-5 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            {{-- Role — the base permission set (per-user overrides live on the Permissions page) --}}
            <div class="border-t border-gray-100 pt-5">
                ...
                <p class="mt-1 text-xs text-[var(--color-muted)]">
                    Gives the base permissions (manage from <a href="{{ route('admin.roles.index') }}" class="font-semibold text-[var(--color-primary)] hover:underline">Roles</a>).
                    @if ($staff->exists) Fine-tune this person on the <a href="{{ route('admin.staff.permissions', $staff) }}" class="font-semibold text-[var(--color-primary)] hover:underline">Permissions page</a>.@else Save first, then set custom permissions from the staff list.@endif
                </p>
            </div>
        </div>
