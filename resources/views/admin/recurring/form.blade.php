...
@section('title', $profile->exists ? 'Edit Recurring' : 'New Recurring Invoice')

@section('content')
<div x-data="{ items: {{ Illuminate\Support\Js::from($items) }} }">
    <a href="{{ route('admin.recurring.index') }}" class="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-[var(--color-muted)] hover:text-[var(--color-heading)]">
        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" d="m15 18-6-6 6-6"/></svg> Back to recurring
    </a>

    <form method="POST" action="{{ $profile->exists ? route('admin.recurring.update', $profile) : route('admin.recurring.store') }}" class="max-w-3xl space-y-6">
        @csrf
        @if ($profile->exists) @method('PUT') @endif

        <div class="space-y-5 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            <h2 class="text-sm font-bold text-[var(--color-heading)]">Schedule</h2>
            <div class="grid gap-5 sm:grid-cols-2">
                <x-admin.field label="Title" name="title" :value="$profile->title" placeholder="e.g. Monthly Hosting" />
                <x-admin.field label="Client" name="client_id" type="select" :value="$profile->client_id" :options="['' => 'No client'] + $clients->pluck('name', 'id')->all()" />
            </div>
            <div class="grid gap-5 sm:grid-cols-4">
                <x-admin.field label="Interval" name="interval" type="select" :value="$profile->interval" :options="\App\Models\RecurringInvoice::INTERVALS" required />
                <x-admin.field label="Next Run" name="next_run_at" type="date" :value="optional($profile->next_run_at)->toDateString()" required />
                <x-admin.field label="Due (days)" name="due_days" type="number" :value="$profile->due_days ?? 14" required />
                <x-admin.field label="Currency" name="currency" type="select" :value="$profile->currency ?? 'USD'" :options="['USD' => 'USD ($)', 'BDT' => 'BDT (৳)', 'EUR' => 'EUR (€)', 'GBP' => 'GBP (£)']" required />
            </div>
            <x-admin.field name="active" type="checkbox" label="Active (auto-generate on schedule)" :value="$profile->active ?? true" />
        </div>
        ...
        <div class="grid gap-5 rounded-xl border border-gray-100 bg-white p-6 shadow-sm sm:grid-cols-2">
            <x-admin.field label="Notes" name="notes" type="textarea" rows="2" :value="$profile->notes" />
            <x-admin.field label="Terms" name="terms" type="textarea" rows="2" :value="$profile->terms" />
            <x-admin.field label="Payment Method" name="payment_method" type="select" :value="$profile->payment_method ?? 'Bank Transfer'" :options="array_combine(\App\Models\ClientInvoice::PAYMENT_METHODS, \App\Models\ClientInvoice::PAYMENT_METHODS)" />
        </div>
        ...
        <div class="flex gap-3">
            <button class="rounded-lg bg-[var(--color-primary)] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[var(--color-primary-hover)]">{{ $profile->exists ? 'Save changes' : 'Create profile' }}</button>
            <a href="{{ route('admin.recurring.index') }}" class="rounded-lg border border-gray-200 px-5 py-2.5 text-sm font-semibold text-[var(--color-muted)] hover:bg-gray-50">Cancel</a>
        </div>
    </form>
</div>
@endsection
