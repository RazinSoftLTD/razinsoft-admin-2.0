...
@section('content')
    ...
    <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
            ...
            <div class="flex items-center gap-3">
                @if ($product->installation_icon)
                    <img src="{{ \App\Http\Resources\ProductResource::media($product->installation_icon) }}" alt="" class="h-10 w-10 rounded-xl border border-gray-100 object-cover">
                @endif
                <h1 class="text-xl font-bold text-[var(--color-heading)]">{{ $product->name }}</h1>
            </div>
            <p class="mt-1 text-sm text-[var(--color-muted)]">Build the plans and the feature comparison shown on the public Installation page.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @endif
            <a href="{{ route('admin.installation-plans') }}" class="rounded-lg border border-gray-200 px-4 py-2.5 text-sm font-semibold text-[var(--color-muted)] transition hover:bg-gray-50">All products</a>
        </div>
    </div>
