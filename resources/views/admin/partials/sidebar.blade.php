@php
    ...
    $canSee = function ($i) use ($user) {
        ...
        if (! empty($i['perm'])) {
            // An array means any one of them is enough — for entries that open a page with
            // several tabs, each gated on its own.
            foreach ((array) $i['perm'] as $perm) {
    ...
    $nav = [
        ...
        ['type' => 'group', 'label' => 'Communication', 'icon' => $ic['chat'], 'items' => [
            ...
            ['label' => 'Website Chat', 'route' => 'admin.site-chat', 'active' => 'admin.site-chat*', 'perm' => 'site_chat.view', 'icon' => $ic['chat'],
             'badge' => \App\Models\SiteChat::where('unread_count', '>', 0)->count() ?: null],
