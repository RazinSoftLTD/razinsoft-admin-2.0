...
@section('content')
    ...
    @switch($relation)
        @case('plans')
            @php $site = rtrim(config('brand.website'), '/'); @endphp

            <div class="mb-5 max-w-3xl rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
                <p class="text-sm font-bold text-[var(--color-heading)]">Checkout links</p>
                <p class="mt-0.5 text-xs text-[var(--color-muted)]">
                    Every plan below has its own link. Send one and the customer lands on the website cart with
                    <strong>{{ $product->name }}</strong> on that plan already in it, one click from paying — no hunting for the
                    right price. The link never carries the price: whatever the plan costs when they open it is what they pay,
                    so an old link can never sell at an old price.
                </p>
                @if ($product->status !== 'published')
                    <p class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                        This product is {{ $product->status }} — its links will not work for customers until it is published.
                    </p>
                @endif
            </div>
            ...
            <div class="max-w-3xl space-y-4">
                @foreach ($product->plans as $pl)
                    <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm" x-data="{ edit: false }">
                        ...
                        {{-- The plan's own checkout link. Quantity is live, so a bulk deal is one
                             field away rather than a hand-edited URL. --}}
                        <div x-show="!edit" x-data="{
                                 qty: 1,
                                 copied: false,
                                 get url() {
                                     const base = @js($site . '/cart?add=' . $product->slug . '&plan=' . $pl->id);
                                     return this.qty > 1 ? base + '&qty=' + this.qty : base;
                                 },
                                 async copy() {
                                     try { await navigator.clipboard.writeText(this.url) }
                                     catch { const b = document.createElement('textarea'); b.value = this.url; b.style.position = 'fixed'; b.style.opacity = '0'; document.body.appendChild(b); b.select(); document.execCommand('copy'); b.remove() }
                                     this.copied = true; setTimeout(() => this.copied = false, 2200);
                                 },
                             }" class="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-gray-100 bg-gray-50/60 p-2.5">
                            <input :value="url" readonly @focus="$event.target.select()"
                                   class="h-9 min-w-0 flex-1 rounded-lg border border-gray-200 bg-white px-3 text-xs text-[var(--color-muted)]" style="min-width:16rem">
                            <label class="flex shrink-0 items-center gap-1.5 text-xs text-[var(--color-muted)]">
                                Qty
                                <input type="number" min="1" max="99" x-model.number="qty" class="h-9 w-16 rounded-lg border border-gray-200 px-2 text-xs">
                            </label>
                            <button type="button" @click="copy()"
                                    :class="copied ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-gray-200 bg-white text-[var(--color-heading)] hover:bg-gray-50'"
                                    class="inline-flex shrink-0 items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold transition">
                                <svg x-show="!copied" class="h-3.5 w-3.5 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10 13a5 5 0 0 0 7 0l3-3a5 5 0 0 0-7-7l-1 1M14 11a5 5 0 0 0-7 0l-3 3a5 5 0 0 0 7 7l1-1"/></svg>
                                <svg x-show="copied" x-cloak class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2.4" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m5 13 4 4L19 7"/></svg>
                                <span x-text="copied ? 'Copied' : 'Copy link'"></span>
                            </button>
                            <a :href="url" target="_blank" rel="noopener" title="Open it the way the customer will see it"
                               class="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-semibold text-[var(--color-heading)] hover:bg-gray-50">
                                <svg class="h-3.5 w-3.5 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14 5h5v5M19 5l-7 7M18 13v6H5V6h6"/></svg>Test
                            </a>
                        </div>
                ...
                @forelse ($product->demos as $d)
                    <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm" x-data="{ edit: false }">
                        ...
                        <form x-show="edit" x-cloak method="POST" action="{{ $edit('demos', $d->id) }}" enctype="multipart/form-data" class="mt-3 space-y-3">
                            ...
                            <div x-data="{ color: @js($d->bg_color ?: '') }">
                                <label class="mb-1.5 block text-sm font-medium">Card background <span class="font-normal text-gray-400">(optional — blank uses the type colour)</span></label>
                                <div class="flex flex-wrap items-center gap-3">
                                    <input type="color" :value="color || '#4f46e5'" @input="color = $event.target.value" class="h-10 w-14 cursor-pointer rounded-lg border border-gray-200 p-1">
                                    <input type="text" name="bg_color" x-model="color" maxlength="20" placeholder="#4f46e5" class="h-10 w-40 rounded-lg border border-gray-200 px-3 text-sm">
                                    ...
                                    <span class="inline-flex items-center gap-2 text-xs text-[var(--color-muted)]">Preview <span class="grid h-8 w-16 place-items-center rounded text-white" :style="`background: ${color || '#4f46e5'}`">Aa</span></span>
                                </div>
                            </div>
