...
@section('content')
    <div x-data="{ edit: { open: false, id: null, name: '' } }">
        <div class="mb-6">
            <h1 class="text-xl font-bold text-[var(--color-heading)]">Departments</h1>
            <p class="mt-1 text-sm text-[var(--color-muted)]">Teams / departments employees belong to.</p>
        </div>

        <div class="grid gap-6 lg:grid-cols-[320px_1fr]">
            {{-- Add form --}}
            <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm lg:sticky lg:top-20 lg:self-start">
                <h2 class="mb-3 text-sm font-bold text-[var(--color-heading)]">Add Departments</h2>
                <form method="POST" action="{{ route('admin.departments.store') }}" class="space-y-3">
                    @csrf
                    <input name="name" value="{{ old('name') }}" required placeholder="e.g. {{ 'department' === 'designation' ? 'Senior Developer' : 'Engineering' }}" class="h-11 w-full rounded-lg border border-gray-200 px-3 text-sm focus:border-[var(--color-primary)] focus:outline-none">
            ...
            <div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.departments.destroy', $item) }}" onsubmit="return confirm('Delete this?')">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="3" class="px-5 py-12 text-center text-gray-400">No Departments yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-show="edit.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="edit.open = false">
            <form method="POST" :action="'{{ url('admin/departments') }}/' + edit.id" class="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
                @csrf @method('PUT')
                <h3 class="text-base font-bold text-[var(--color-heading)]">Edit Departments</h3>
