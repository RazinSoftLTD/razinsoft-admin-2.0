...
@section('content')
    ...
    <form method="GET" class="mb-4">
        <input name="search" value="{{ request('search') }}" placeholder="Search name, email, phone, company…" class="h-10 w-full max-w-md rounded-lg border border-gray-200 px-3 text-sm focus:border-[var(--color-primary)] focus:outline-none">
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                <thead class="bg-gray-50 text-xs uppercase tracking-wide text-gray-400">
                    <tr>
                        <th class="px-5 py-3 font-semibold">Client ID</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($clients as $c)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 text-[var(--color-muted)]">{{ $c->phone ?? '—' }}</td>
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.clients.edit', $c) }}" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-[var(--color-primary)]" title="Edit">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400">No clients yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
