        ...
        <div class="space-y-4">
            ...
            <div class="rounded-xl border border-gray-100 bg-white shadow-sm">
                ...
                <div class="divide-y divide-gray-50">
                    @forelse ($deal->milestones as $ms)
                        <div class="flex flex-wrap items-center justify-between gap-2 px-5 py-3" x-data="{ edit: false }">
                            ...
                            @if ($canEditDeal)
                                ...
                                <form x-show="edit" x-cloak method="POST" action="{{ route('admin.deals.milestones.update', [$deal, $ms]) }}" class="flex w-full flex-col gap-2">
                                    @csrf @method('PUT')
                                    <input type="text" name="title" required maxlength="150" value="{{ $ms->title }}" class="h-9 w-full rounded-lg border-gray-200 text-sm" placeholder="Title">
                                    <div class="flex gap-2"><input type="number" name="amount" step="0.01" min="0" value="{{ (float) $ms->amount }}" class="h-9 w-full rounded-lg border-gray-200 text-sm" placeholder="Amount"><input type="date" name="due_date" value="{{ $ms->due_date?->format('Y-m-d') }}" class="h-9 w-full rounded-lg border-gray-200 text-sm"></div>
                    ...
                    @if ($canEditDeal)
                        <form method="POST" action="{{ route('admin.deals.milestones.store', $deal) }}" class="flex flex-col gap-2 bg-gray-50/60 px-5 py-3">
                            @csrf
                            <input type="text" name="title" required maxlength="150" class="h-9 w-full rounded-lg border-gray-200 text-sm" placeholder="Milestone title (e.g. Advance payment)">
                            <div class="flex gap-2"><input type="number" name="amount" step="0.01" min="0" class="h-9 w-full rounded-lg border-gray-200 text-sm" placeholder="Amount"><input type="date" name="due_date" class="h-9 w-full rounded-lg border-gray-200 text-sm"></div>
