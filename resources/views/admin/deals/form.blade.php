    ...
    <form method="POST" action="{{ $deal->exists ? route('admin.deals.update', $deal) : route('admin.deals.store') }}" class="max-w-2xl">
        ...
        <div class="space-y-5 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
            ...
            <div class="grid gap-5 sm:grid-cols-3">
                ...
                <x-admin.field label="Currency" name="currency" type="select" :value="$deal->currency ?? 'USD'" :options="['USD' => 'USD ($)', 'BDT' => 'BDT (৳)', 'EUR' => 'EUR (€)', 'GBP' => 'GBP (£)']" required />
            </div>
            ...
            <x-admin.field label="Notes" name="notes" type="textarea" rows="3" :value="$deal->notes" />
        </div>
