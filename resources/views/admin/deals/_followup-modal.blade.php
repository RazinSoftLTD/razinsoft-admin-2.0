{{-- Shared Follow-up popup modal — opened via window event `open-followup` with { action, dealTitle }. --}}
<div x-data="{
        open: false, action: '', dealTitle: '', title: '', note: '', date: '', time: '10:00',
     ...
     @open-followup.window="open = true; action = $event.detail.action; dealTitle = $event.detail.dealTitle; title = ''; note = ''; date = ''; time = '10:00'"
    ...
    <div x-show="open" class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 pt-20" @click.self="open = false">
        <div x-show="open"
            ...
            <div class="relative flex items-start gap-3 bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-hover)] px-5 py-4 text-white">
                ...
                <div class="min-w-0 flex-1">
                    <h3 class="text-base font-bold">Schedule Follow-up</h3>
                    <p class="mt-0.5 truncate text-xs text-white/80" x-text="dealTitle"></p>
                </div>
            ...
            <form method="POST" :action="action" data-turbo="false" class="px-5 py-5">
                ...
                <div class="space-y-4">
                    {{-- Title --}}
                    <div>
                        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-400">Title</label>
                        <input type="text" name="follow_up_title" x-model="title" maxlength="255" placeholder="e.g. Call to discuss proposal"
                               class="h-11 w-full rounded-xl border-gray-200 bg-gray-50 text-sm transition focus:border-[var(--color-primary)] focus:bg-white focus:ring-2 focus:ring-[var(--color-primary)]/20">
                    </div>

                    {{-- Note --}}
                    <div>
                        <label class="mb-1.5 block text-xs font-bold uppercase tracking-wide text-gray-400">Short description</label>
                        <textarea name="follow_up_note" x-model="note" rows="3" maxlength="2000" placeholder="What is this follow-up about?"
                                  class="w-full rounded-xl border-gray-200 bg-gray-50 text-sm transition focus:border-[var(--color-primary)] focus:bg-white focus:ring-2 focus:ring-[var(--color-primary)]/20"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mb-2 flex flex-wrap gap-1.5">
                            @foreach (['Today' => 0, 'Tomorrow' => 1, 'In 3 days' => 3, 'Next week' => 7] as $label => $days)
                                <button type="button" @click="setDate({{ $days }})"
                                        :class="isDay({{ $days }}) ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-gray-200 bg-white text-[var(--color-muted)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]'"
                                        class="rounded-lg border px-2.5 py-1 text-xs font-semibold transition">{{ $label }}</button>
                            @endforeach
                        </div>
                        ...
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            @foreach (['9:00 AM' => '09:00', '12:00 PM' => '12:00', '3:00 PM' => '15:00', '6:00 PM' => '18:00'] as $label => $t)
                                <button type="button" @click="time = '{{ $t }}'"
                                        :class="time === '{{ $t }}' ? 'border-[var(--color-primary)] bg-[var(--color-primary-soft)] text-[var(--color-primary)]' : 'border-gray-200 bg-white text-[var(--color-muted)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]'"
                                        class="rounded-lg border px-2.5 py-1 text-xs font-semibold transition">{{ $label }}</button>
                            @endforeach
                        </div>
                        ...
                        <input type="hidden" name="next_follow_up_at" :value="dt">
                    </div>
                </div>
