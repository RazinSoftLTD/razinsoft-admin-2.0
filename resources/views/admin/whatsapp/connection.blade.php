        ...
        <div>
            ...
            <p class="mt-1 text-sm text-[var(--color-muted)]">
                @if ($account->isCloudApi())
                    Meta Cloud API. Verifying asks Meta whether the credentials on this number work, and reads back the number it answers for.
                @else
                    Scan the QR with the phone for this number ({{ $account->name }}). Each number keeps its own session &amp; inbox.
                @endif
            </p>
        </div>
        ...
        <div x-show="state === 'disconnected' || state === 'connecting'" x-cloak class="text-center">
            <template x-if="!configured">
                <div class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                    @if ($account->isCloudApi())
                        Add this number's Phone Number ID and Access Token in <a href="{{ route('admin.whatsapp-settings') }}" class="font-semibold underline">Settings</a> first, then come back to verify.
                    @else
            ...
            <template x-if="configured">
                <div>
                    ...
                    <p class="mt-4 text-sm text-gray-500" x-text="message || @js($account->isCloudApi() ? 'Not verified yet. Check the credentials against Meta.' : 'Not connected. Click connect to generate a QR code.')"></p>
                    <button type="button" @click="connect()" :disabled="busy" class="mt-6 rounded-lg bg-emerald-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-emerald-600 disabled:opacity-50">
                        <span x-text="busy ? @js($account->isCloudApi() ? 'Verifying…' : 'Starting…') : @js($account->isCloudApi() ? 'Verify with Meta' : 'Connect WhatsApp')"></span>
                    </button>
                </div>
            </template>
        </div>
    ...
    <script>
        function waConnection() {
            return {
                ...
                qr: null, number: @js($account->display_number), configured: {{ $account->isConfigured() ? 'true' : 'false' }},
