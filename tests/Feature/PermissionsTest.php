...
class PermissionsTest extends TestCase
{
    ...
    /** Razin AI is its own module now — the WhatsApp Config permission no longer opens it. */
    public function test_razin_ai_has_its_own_permission(): void
    {
        $role = Role::create(['name' => 'WA Config', 'permissions' => ['whatsapp.settings' => 'all']]);
        $staff = User::create(['name' => 'S', 'email' => 'rai1@test.local', 'password' => 'password', 'role' => 'staff', 'role_id' => $role->id]);
        $this->actingAs($staff);

        $this->get('/admin/razin-ai')->assertForbidden();

        // Granting the page does not also grant changing how the assistant speaks.
        $staff->update(['permissions' => ['razin_ai.view' => 'all']]);
        $this->get('/admin/razin-ai')->assertOk();
        $this->post('/admin/razin-ai', ['mode' => 'off'])->assertForbidden();
        $this->post('/admin/razin-ai/faqs', ['keywords' => 'x', 'reply' => 'y'])->assertForbidden();

        $staff->update(['permissions' => ['razin_ai.view' => 'all', 'razin_ai.faqs' => 'all']]);
        $this->post('/admin/razin-ai/faqs', ['keywords' => 'demo', 'reply' => 'here it is'])->assertRedirect();
        $this->assertDatabaseHas('ai_faqs', ['keywords' => 'demo']);
    }

    /** A section override with an owned/added scope used to be silently thrown away on save. */
    public function test_a_section_scope_override_survives_saving(): void
    {
        $admin = $this->admin();
        $staff = User::create(['name' => 'S', 'email' => 'rai2@test.local', 'password' => 'password', 'role' => 'staff']);

        $this->actingAs($admin)->put("/admin/staff/{$staff->id}/permissions", ['override' => [
            'clients.projects' => 'owned',      // a scopable SECTION
            'razin_ai.view' => 'all',
            'clients.view' => '',               // inherit → not stored
        ]])->assertRedirect();
